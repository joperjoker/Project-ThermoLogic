...
| Can it give a *hard* guarantee, not just a soft penalty? | **Yes** — a discrete backstop returns a verified-valid state or *proves* UNSAT | §6.6 |
...
intent conflict — all label-free. Run against **five configurations generated by
Claude in-session** (real model JSON, since this environment has no external API
key), it flags four, verifiably repairs three, and proves one — a "public
analytics dashboard backed by raw customer PII" — unsatisfiable.

### 6.6 From a soft penalty to a hard guarantee

The paper's sharpest self-criticism has been that soft energy makes contradictions
*expensive, not impossible* — gradient descent can stall in a shallow, still-
invalid basin, so `repair` could hand back an output that is not actually rule-
satisfying (the `RuntimeWarning` some configs triggered). A reliability tool
cannot ship that. We close the gap with a **discrete backstop** (`thermologic/
solver.py`): after the soft energy descent warm-starts a state, a **min-conflicts
local search** over the free atoms drives the *crisp* violation count to zero, and
the result is **verified** before return. Two guarantees follow, both *sound*:

* `LogicEnergy.repair(..., guarantee=True)` returns a state that is **checked
  rule-valid**, or raises — it never silently returns an invalid "repair."
* When no valid completion exists under the fixed inputs, a sound UNSAT check
  returns the **conflicting rule(s)** (`satisfiability()` / `solve()` raise
  `UnsatisfiableError` with the core), so "this request is self-contradictory" is
  a *proof*, not a guess.

Local search is sound-for-SAT but incomplete-for-UNSAT, so we never claim UNSAT
from search failure alone — only from the sound conflict check — and the status is
reported explicitly (`already_valid` / `repaired` / `unsat` / `unknown`). The
guarantee is exactly as strong as the rule set: for the config domain, whose rules
fully capture the policy, it is a true hard guarantee. This converts the honest
limitation below from "open" to "addressed for propositional rule sets."
...
- **Scale & scope.** 11 atoms, propositional Horn-style rules, synthetic data. No
  first-order variables, no defeasible/non-monotonic rules, no live hosted LLM
  (the in-session guardrail runs on Claude-generated JSON, §6.5).
- **Soft vs. hard guarantee — now addressed.** Soft energy alone makes
  contradictions *expensive, not impossible*; the discrete backstop of §6.6
  (`guarantee=True`) upgrades this to a **verified** hard guarantee for
  propositional rule sets — returning a checked-valid state or a *proof* of
  unsatisfiability. Local search remains incomplete for UNSAT (sound but not a
  decision procedure), so very large/hard UNSAT instances can return `unknown`.
